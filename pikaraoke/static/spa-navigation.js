...
(function() {
    ...
    function initUsernameHandler() {
        ...
        $(document).on('click', '#current-user', async function(e) {
            ...
            let singers = [];
            try {
                const resp = await fetch('/singers');
                singers = await resp.json();
            } catch (_) {}

            let name;
            if (singers.length > 1) {
                const list = singers.map((s, i) => `${i + 1}. ${s}`).join('\n');
                const input = window.prompt(
                    `Switch singer (enter number or type a new name):\n${list}`,
                    currentName
                );
                if (input === null) { $(this).blur(); return; }
                const idx = parseInt(input, 10) - 1;
                name = (idx >= 0 && idx < singers.length) ? singers[idx] : input.trim();
            } else {
                var promptMsg = (window.i18n && window.i18n.promptChangeUsername)
                    ...
                    : "Change name (shows on queued songs). Current: " + currentName;
